...
public class FootballRanker {
    ...
    private double printRootMeanSquareError(List<String> humanRanks,
        ...
        for(AllPathsInfo curr : paths) {
            predictedRanks.add(curr.getName());
        }
        double runningSum = getRunningSum(humanRanks, paths, showResults, predictedRanks);
        double result = Math.sqrt(runningSum / humanRanks.size());
        if (showResults) {
            System.out.printf("Root Mean Square Error: %." + NUM_DECIMALS + "f\n", result);
        }
        return Math.round(ROUNDING_FACTOR * result) / ROUNDING_FACTOR;
    }

    /**
    * Calculates the running sum of the root mean square error between human ranks
    * and predicted ranks
    *  @param humanRanks List of human ranks in string format
    *  @param paths Set of AllPathsInfo objects containing the predicted ranks for each item
    *  @param showResults flag indicating whether to print results to console
    *  @param predictedRanks List of predicted ranks in string format
    *  @return double value representing the running sum of the root mean square error
    */
    private double getRunningSum(List<String> humanRanks, TreeSet<AllPathsInfo> paths,
                                 boolean showResults, ArrayList<String> predictedRanks) {
        double runningSum = 0.0;
        int index = 1;
        for (String rank: humanRanks) {
            String curr = humanRanks.get(index - INDEX_ADJUSTMENT);
            runningSum += Math.pow(index - getRank(paths, rank), SQUARE);
            if (showResults) {
                System.out.printf("%-" + PADDING + "s", curr);
                System.out.println(" - actual rank: " +
                        (index) + " predicted rank: " +
                        (predictedRanks.indexOf(curr) + INDEX_ADJUSTMENT));
            }
            index++;
        }
        return runningSum;
    ...
    private int getRank(TreeSet<AllPathsInfo> paths, String curr) {
        int index = 0;
        for (AllPathsInfo path: paths) {
            index++;
            if (path.getName().equals(curr)){
                return index;
            }
        }
        return index;
