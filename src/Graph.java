...
public class Graph {
    ...
    public void dijkstra(String startName) {
        ...
        clearAll();
        currentStartVertexName = startName;
        PriorityQueue<Path> myQueue = new PriorityQueue<Path>();
        Vertex start = vertices.get(currentStartVertexName);
        start.weightedCostFromStartVertex = 0;
        start.numEdgesFromStartVertex = 0;
        Path startingPath = new Path(start, 0);
        startingPath.add(start);
        myQueue.add(startingPath);
        while(!myQueue.isEmpty()) {
            Path currentPath = myQueue.remove();
            Vertex currentVertex = currentPath.dest;
            performDijkstra(myQueue, currentPath, currentVertex);
        }
    }

    /**
     * Perform Dijkstra's algorithm on a given vertex, updating the shortest path to all its
     * neighbors that haven't been visited yet.
     * @param myQueue A PriorityQueue of Paths to process.
     * @param currentPath The current path being processed.
     * @param currentVertex The current vertex being processed.
     */
    private static void performDijkstra(PriorityQueue<Path> myQueue,
                                        Path currentPath, Vertex currentVertex) {
        if (currentPath.dest.scratch < 1) {
            currentVertex.scratch++;
            for (Edge currentEdge: currentVertex.adjacent) {
                Vertex neighbor = currentEdge.dest;
                Double distance = currentPath.weightedCostOfPath + currentEdge.cost;
                if(neighbor.weightedCostFromStartVertex > distance) {
                    neighbor.prev = currentVertex;
                    neighbor.weightedCostFromStartVertex =
                            currentVertex.weightedCostFromStartVertex + currentEdge.cost;
                    neighbor.numEdgesFromStartVertex =
                            currentVertex.numEdgesFromStartVertex + 1;

                    myQueue.add(new Path(neighbor, neighbor.weightedCostFromStartVertex));
                }
            }
        }
    }
    ...
    public void findAllPaths(boolean weighted) {
        allPathsFound = true;
        longest = new Path();
        for (String curr: vertices.keySet()) {
            Vertex v = vertices.get(curr);
            v.clearPathInfo();
            // Find shortest path
            if (weighted) {
                dijkstra(curr);
                ...
                findUnweightedShortestPath(curr);
            }
            findPathStats(v);
        }
    }
    ...
    /**
     * Loops through all vertices in a graph
     * If 2 vertices are unique and have a connecting path, then inside the vertex object,
     * totalUnweightedPathLength, totalWeightedPathLength, and numVertexConnected are all updated.
     * @param v Current vertex object being inspected.
     */
    private void findPathStats(Vertex v) {
        for (Vertex v2: vertices.values()) {
            if(!v2.name.equals(v.name) && v2.weightedCostFromStartVertex < INFINITY) {
                v.totalUnweightedPathLength += v2.numEdgesFromStartVertex;
                v.totalWeightedPathLength += v2.weightedCostFromStartVertex;
                v.numVertexConnected++;
                if (v2.weightedCostFromStartVertex > longest.weightedCostOfPath)
                    longest = getPath(v2.name);
            }
        }
    }
